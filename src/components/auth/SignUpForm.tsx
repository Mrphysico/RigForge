...
import { User, Mail, Lock, AlertCircle, ArrowRight, Loader2, Eye, EyeOff, CheckCircle2 } from 'lucide-react';
...
export const SignUpForm: React.FC<SignUpFormProps> = ({
  ...
  const { signup, startGoogleLogin } = useAuthStore();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (res.success) {
      setSuccessMessage('Account created successfully! Welcome to RigForge.');
      if (onNotification) {
        onNotification('Account created successfully! Welcome to RigForge.');
      }
    } else {
      setError(res.error || 'Failed to create your account. Please try again.');
    }
  };
  ...
  const handleGoogleClick = async () => {
    ...
    if (result.success) {
      if (onNotification) onNotification('Account registered successfully with Google!');
    ...
    <div className="space-y-4">
        ...
        <div>
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2.5 text-xs sm:text-sm bg-slate-50 text-slate-900 placeholder-slate-400 rounded-xl border border-slate-200 focus:outline-none focus:border-[#0066ff] focus:ring-1 focus:ring-[#0066ff] transition-colors"
            />
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-10 pr-8 py-2.5 text-xs bg-slate-50 text-slate-900 placeholder-slate-400 rounded-xl border border-slate-200 focus:outline-none focus:border-[#0066ff] transition-colors"
            />
            <button
              type="button"
      ...
      <div className="grid grid-cols-3 gap-2.5">
        <button
          ...
          disabled={loading || googleLoading}
          className="col-span-1 flex items-center justify-center gap-2 py-2 px-3 rounded-xl bg-slate-50 hover:bg-slate-100 border border-slate-200 text-slate-700 text-xs font-semibold transition-all disabled:opacity-50"
        >
          {googleLoading ? (
        ...
        <button
          type="button"
          onClick={() => onNotification?.('GitHub registration coming soon.')}
          className="col-span-1 flex items-center justify-center gap-1.5 py-2 px-3 rounded-xl bg-slate-50 hover:bg-slate-100 border border-slate-200 text-slate-700 text-xs font-semibold transition-all"
        >
        ...
        <button
          type="button"
          onClick={() => onNotification?.('Discord registration coming soon.')}
          className="col-span-1 flex items-center justify-center gap-1.5 py-2 px-3 rounded-xl bg-slate-50 hover:bg-slate-100 border border-slate-200 text-slate-700 text-xs font-semibold transition-all"
        >
